...
class BuscaChavePixEndpointTest(
    ...
    fun `deve buscar uma chave pix que nao existe no banco, mas existe no BCB`() {
        ...
        Mockito.`when`(sistemaPixBCBClient.buscaPor(key = "90216262097")).thenReturn(
            HttpResponse.ok(chavePixDetalheResponse())
        )

        // acao
        val response = clientGrpc.buscar(BuscaChavePixRequest.newBuilder().setChave("90216262097").build())

        with(response) {
            assertNotNull(this)
            assertEquals("90216262097", chave.chave)
...
fun chavePix(): ChavePix {
    return ChavePix(
        ...
        conta = ContaAssociada(
            ...
            nomeDoTitular = "Caio Rodrigues",
            cpfDoTitular = "902.162.620-97",
            instituicao = "ITAÚ"
        )
    )
}
...
fun chavePixDetalheResponse(): ChavePixDetalheResponse {
    return ChavePixDetalheResponse(
        ...
        owner = Owner(TypeRequest.LEGAL_PERSON, name = "Caio", taxIdNumber = "45755411840"),
